...
def next_snapshot_and_version(latest_snapshot_version: Optional[int]) -> Optional[Tuple[DataFrame, int]]:
    ...
    if latest_snapshot_version is None:
        df = spark.sql("""
            SELECT * FROM {catalog}.{bronze_schema}.supplier 
            WHERE snapshot_id = (SELECT min(snapshot_id) FROM {catalog}.{bronze_schema}.supplier)
        """)
        
        min_snapshot_id = spark.sql("""
            SELECT min(snapshot_id) as min_id FROM {catalog}.{bronze_schema}.supplier
        """).collect()[0].min_id
        ...
        next_snapshot_result = spark.sql(f"""
            ...
            FROM {catalog}.{bronze_schema}.supplier 
